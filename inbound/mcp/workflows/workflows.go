...
func (h handler) signal(
	...
	selection, err := h.selection(ctx, input.Workflows)
	if err != nil {
		return nil, actionOutput{}, err
	}
...
func (h handler) selection(ctx context.Context, input workflowSelectionInput) (ports.WorkflowSpec, error) {
	...
	if input.Filter != nil {
		filter, err := filterFromInput(h.svc.SearchSchemas(ctx).Combined(), *input.Filter)
		if err != nil {
